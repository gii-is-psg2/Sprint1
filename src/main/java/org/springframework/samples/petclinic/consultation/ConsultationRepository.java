...
//	public List<Consultation> findConsultationsByVet(@Param("vetId") int vetId);
	...
	public List<Consultation> findAllByClinicOwnerUserIdAndIsClinicComment(int userId, boolean isComment);

	@Query("SELECT c FROM Consultation c WHERE c.owner.clinic.id = :clinicId AND c.isClinicComment = :isComment")
	public List<Consultation> findAllByClinicIdAndIsClinicComment(int clinicId, boolean isComment);
